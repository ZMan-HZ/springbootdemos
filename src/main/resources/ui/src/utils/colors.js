...
// Custom colors specific to Dashboards, includes the new "grays".
const newColors = {
    ...
    for (var colorName in overrides) {
        if (overrides.hasOwnProperty(colorName)) {
            if (!/^#([A-Fa-f0-9]{3}$)|([A-Fa-f0-9]{6}$)/.test(overrides[colorName]))
                throw new Error('Please provide a hexadecimal color value if you are trying to override the Dashboards colors.');
